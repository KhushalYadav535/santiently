...
const DEFAULT_WORDS = ["Santiently", "Innovations", "Enterprise AI"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  // Vectr Signature Tech Palette: Electric Cobalt, Deep Ink, Steel
  const colorPalette = [
    { r: 15, g: 50, b: 220 },  // Cobalt Blue (#0F32DC)
    { r: 5, g: 4, b: 25 },     // Deep Ink (#050419)
    { r: 88, g: 87, b: 101 },  // Steel Slate (#585765)
  ]
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Calculate font size dynamically so words like Santiently fit on all screens
    let fontSize = 96
    offscreenCtx.font = `bold ${fontSize}px Inter, -apple-system, sans-serif`
    const maxTextWidth = canvas.width * 0.88
    while (offscreenCtx.measureText(word).width > maxTextWidth && fontSize > 28) {
      fontSize -= 4
      offscreenCtx.font = `bold ${fontSize}px Inter, -apple-system, sans-serif`
    }
    ...
    const currentColorChoice = colorPalette[wordIndexRef.current % colorPalette.length]
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0) {
        ...
        particle.targetColor = currentColorChoice
  ...
  const animate = () => {
    ...
    // Background with soft light trail
